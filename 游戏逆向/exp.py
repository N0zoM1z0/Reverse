"""
mn_tt_klogo00.omv
"""

# with open("mn_tt_klogo00.omv", "rb") as f:
#     fout = open("extract.ogg", "wb")
#     data = f.read()
#     data = data[0x2a16:0x2a16+0xC00]
#     fout.write(data)
#     print("[+] Done")

"""
read.sav
"""

key = []
with open("read.sav", "rb") as f:
    data = f.read()
v12 = int.from_bytes(data[8:12],"little")
v26 = int.from_bytes(data[12:16],"little")
int_data = [int(data[i]) for i in range(len(data))]
for i in range(0x10,0x10+v12):
    int_data[i] = int_data[i] ^ key[(i-0x10)&0xFF]

# f = open("new_read.sav", "wb")
# for i in range(len(int_data)):
#     f.write(int_data[i].to_bytes(1, "little"))

data = int_data
v25 = int.from_bytes(data[20:24],"little")
data = data[24:]
v4 = data[0] + 256
v5 = 1
new_data = [0] * v25
i = 0
while True:
    if i >= v25:
        break
    if v4 == 1:
        v4 = data[v5] + 256
        v5 += 1  
    if (v4 & 1) != 0:
        new_data[i] = data[v5]
        i += 1
        v5 += 1
    else:
        v6 = i-(v5>>4)
        for j in range((v5&0xF)+2,0,-1):
            new_data[i] = data[v6]
            i += 1
            v6 += 1
        v5 += 2
    v4 >>= 1
print(new_data)